...
def from_hparams(
    ...
    **kwargs,
    ...
    if savedir is None:
        ...
        savedir = f"./singer_identity/pretrained_models/{clsname}-{hashlib.md5(source.encode('UTF-8', errors='replace')).hexdigest()}"
...
def from_scripted(filename, source, savedir=None):
    ...
    if savedir is None:
        savedir = f"./singer_identity/pretrained_models/{filename}-{hashlib.md5(source.encode('UTF-8', errors='replace')).hexdigest()}"
